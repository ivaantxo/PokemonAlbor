...
static void GetTMNumberAndMoveString(u8 * dest, u16 itemId)
{
    ...
    if (itemId >= ITEM_HM01)
    {
        ...
        StringAppend(gStringVar4, gText_Clear01);
...
static void TMCase_MoveCursor_UpdatePrintedTMInfo(u16 itemId)
{
    ...
    else
    {
        ...
        if (sTMCaseDynamicResources->typeIconSpriteId == 0xFF)
        {
		    sTMCaseDynamicResources->typeIconSpriteId = CreateSpriteAtEnd(&gSpriteTemplate_MoveTypes, 72, 109, 0);
		    sprite = &gSprites[sTMCaseDynamicResources->typeIconSpriteId];
		    StartSpriteAnim(sprite, gBattleMoves[move].type);
		    sprite->oam.paletteNum = sMoveTypeToOamPaletteNum[gBattleMoves[move].type];
        }
